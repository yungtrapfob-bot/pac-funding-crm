...
- Expected rep result: direct URL is blocked server-side by the admin layout/page guard and middleware redirects the rep to `/dashboard`.
...
- Reps cannot view company-wide admin dashboard metrics, the processing queue, user management, or funder master screens.
- Reps cannot view deal-router/funder-master guideline data, processing submission desk data, payout/renewal calculations, gross commission fields, or internal workflow notes on deal detail pages.
...
- Lead activity rows are removed before the lead delete to avoid deployed databases with missing cascade behavior causing a dashboard error.
...
7. As an admin, create a disposable non-converted hot lead from `/hot-leads/new`, delete it from the hot-leads actions menu, and confirm `/hot-leads?deleted=1` loads with the success banner.

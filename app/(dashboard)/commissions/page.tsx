    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-md border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>{data?.map((row) => <tr key={row.id} className="border-t border-border"><td className="p-2">{(row as any).deals?.business_name}</td><td className="p-2">{(row as any).profiles?.full_name ?? 'You'}</td><td className="p-2">{row.split_role}</td><td className="p-2">{Number(row.split_percent) * 100}%</td><td className="p-2">${Number(row.commission_amount).toLocaleString()}</td></tr>)}</tbody>
        </table>
      </div>
    </div>
  );

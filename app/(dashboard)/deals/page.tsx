...
export default async function DealsPage({ searchParams }: { searchParams: { q?: string; stage?: string; rep?: string } }) {
  ...
  <Card><form className="grid grid-cols-1 gap-2 md:grid-cols-5"><Input name="q" defaultValue={q} placeholder="Search business, owner, email"/><Input name="stage" defaultValue={stage} placeholder="Filter by exact stage"/><Input name="rep" defaultValue={rep} placeholder="Assigned rep id"/><button className="rounded-md border border-border px-3 py-2 text-sm">Filter</button></form></Card>

...
export default async function HotLeadsPage() {
  ...
  const hotLeads = (data ?? []) as HotLeadRowWithProfile[];
      ...
      <Card className="p-0">
        {!hotLeads.length ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{hotLeads.map((lead) => <tr key={lead.id} className="border-t border-border"><td className="p-2">{lead.business_name}</td><td className="p-2">{lead.owner_name}</td><td className="p-2">{lead.profiles?.full_name}</td><td className="p-2">{lead.next_follow_up_date || '—'}</td></tr>)}</tbody>
          </table>
        )}
      </Card>
    </div>

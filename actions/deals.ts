...
function resolveInternalOwnerId(record: { owner_profile_id?: string | null; assigned_rep_id?: string | null }) {
  return record.owner_profile_id ?? record.assigned_rep_id ?? null;
}

export async function createHotLead(_: HotLeadFormState, formData: FormData): Promise<HotLeadFormState> { const supabase = await createClient(); const assignedRepId = await resolveProfileIdForUser(); const parsed = hotLeadSchema.safeParse(Object.fromEntries(formData)); if (!parsed.success) return { status: 'error', message: 'Please fix the highlighted fields.', fieldErrors: parsed.error.flatten().fieldErrors }; const { next_follow_up_date, ...rest } = parsed.data; const { data: lead, error } = await supabase.from('hot_leads').insert({ ...rest, next_follow_up_date: next_follow_up_date ? new Date(next_follow_up_date).toISOString() : null, assigned_rep_id: assignedRepId, owner_profile_id: assignedRepId, last_contact_date: new Date().toISOString().slice(0, 10) }).select('id').single(); if (error || !lead) return { status: 'error', message: error?.message ?? 'Failed to create lead.' }; revalidatePath('/dashboard'); revalidatePath('/hot-leads'); redirect(`/hot-leads/${lead.id}?created=1`); }

export async function updateHotLead(formData: FormData) { const supabase = await createClient(); const p = Object.fromEntries(formData); const { data: existingLead } = await supabase.from('hot_leads').select('assigned_rep_id,owner_profile_id').eq('id', String(p.id)).maybeSingle(); const ownerId = resolveInternalOwnerId(existingLead ?? {}) ?? (await resolveProfileIdForUser()); const { error } = await supabase.from('hot_leads').update({ business_name: p.business_name, owner_name: p.owner_name, phone: p.phone, email: p.email, follow_up_status: followUpStatusSchema.parse(p.follow_up_status), next_follow_up_date: p.next_follow_up_date ? new Date(String(p.next_follow_up_date)).toISOString() : null, notes: p.notes, outcome_tag: p.outcome_tag, last_contact_date: new Date().toISOString().slice(0, 10), assigned_rep_id: ownerId, owner_profile_id: ownerId }).eq('id', String(p.id)); if (error) throw new Error(error.message); revalidatePath('/hot-leads'); revalidatePath(`/hot-leads/${p.id}`); redirect(`/hot-leads/${p.id}?saved=hot_lead`); }

export async function convertHotLeadToDeal(formData: FormData) { const supabase = await createClient(); const leadId = String(formData.get('hot_lead_id')); const { data: lead, error } = await supabase.from('hot_leads').select('*').eq('id', leadId).single(); if (error || !lead) throw new Error('Lead not found'); const { data: existingDeal } = await supabase.from('activities').select('deal_id').eq('hot_lead_id', lead.id).eq('activity_type', 'hot_lead_converted').order('created_at', { ascending: false }).limit(1).maybeSingle(); if (existingDeal?.deal_id) { redirect(`/deals/${existingDeal.deal_id}`); } const ownerId = resolveInternalOwnerId(lead) ?? (await resolveProfileIdForUser()); const { data: deal, error: dealErr } = await supabase.from('deals').insert({ business_name: lead.business_name, owner_name: lead.owner_name, phone: lead.phone, email: lead.email, industry: lead.industry, monthly_revenue: lead.monthly_revenue, time_in_business_months: lead.time_in_business_months, state: lead.state, positions: lead.positions, nsf_count: lead.nsf_count, deposits: lead.deposits, fico: lead.fico, notes: lead.notes, assigned_rep_id: ownerId, owner_profile_id: ownerId, converted_from_hot_lead_id: lead.id }).select('id').single(); if (dealErr || !deal) throw new Error(dealErr?.message ?? 'Failed to convert lead'); const { data: { user } } = await supabase.auth.getUser(); if (user?.id) { await supabase.from('activities').insert({ hot_lead_id: lead.id, deal_id: deal.id, actor_id: user.id, activity_type: 'hot_lead_converted', details: { source: 'quick_convert' } }); } revalidatePath('/dashboard'); revalidatePath('/hot-leads'); revalidatePath('/deals'); redirect(`/deals/${deal.id}`); }
...
export async function submitHotLeadConversion(
  ...
  const leadId = String(formData.get('hot_lead_id') ?? '');
  const { data: lead, error: leadError } = await supabase.from('hot_leads').select('id,assigned_rep_id,owner_profile_id').eq('id', leadId).maybeSingle();
  ...
  const assignedRepId = resolveInternalOwnerId(lead) ?? (await resolveProfileIdForUser());
  ...
  const payload = {
    ...
    owner_profile_id: assignedRepId,
...
export async function createDeal(formData: FormData) { /* unchanged */
 const supabase = await createClient(); const assignedRepId = await resolveProfileIdForUser(); const payload = Object.fromEntries(formData); const { error } = await supabase.from('deals').insert({ business_name: payload.business_name, owner_name: payload.owner_name, phone: payload.phone, email: payload.email, industry: payload.industry, monthly_revenue: Number(payload.monthly_revenue), time_in_business_months: Number(payload.time_in_business_months), state: payload.state, positions: Number(payload.positions), nsf_count: Number(payload.nsf_count), deposits: Number(payload.deposits), fico: Number(payload.fico), notes: payload.notes, internal_notes: payload.internal_notes, assigned_rep_id: assignedRepId, owner_profile_id: assignedRepId }); if (error) throw new Error(error.message); revalidatePath('/deals'); }
...
export async function selectOffer(formData: FormData) {
  ...
  if (offer.status === 'declined') throw new Error('Declined offers cannot be selected');
  ...
  const { data: updatedOfferRows, error: offerUpdateError } = await adminSupabase
    .from('offers')

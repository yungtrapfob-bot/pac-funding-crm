...
export async function selectOffer(formData: FormData) { const supabase = await createClient(); const p = Object.fromEntries(formData); const offerId = String(p.offer_id); const dealId = String(p.deal_id); const { data: offerRows, error: offerError } = await supabase.from('offers').select('id,deal_id,status,created_at').eq('id', offerId).eq('deal_id', dealId).order('created_at', { ascending: false }).limit(2); if (offerError) throw new Error(offerError.message); const offer = offerRows?.[0] ?? null; if (!offer) throw new Error('Offer not found for this deal'); if (offer.status === 'declined') throw new Error('Declined offers cannot be selected'); const { error: resetError } = await supabase.from('offers').update({ status: 'open' }).eq('deal_id', dealId).eq('status', 'accepted').neq('id', offerId); if (resetError) throw new Error(resetError.message); const { error: offerUpdateError } = await supabase.from('offers').update({ status: 'accepted' }).eq('id', offerId).eq('deal_id', dealId); if (offerUpdateError) throw new Error(offerUpdateError.message); const { data: persistedOffer, error: persistedOfferError } = await supabase.from('offers').select('id,status').eq('id', offerId).eq('deal_id', dealId).maybeSingle(); if (persistedOfferError) throw new Error(persistedOfferError.message); if (!persistedOffer || String(persistedOffer.status ?? '').toLowerCase() !== 'accepted') throw new Error('Selected offer status did not persist as accepted'); const { error: dealError } = await supabase.from('deals').update({ current_stage: toDbPipelineStage('Contracts Out') }).eq('id', dealId); if (dealError) throw new Error(dealError.message); revalidatePath(`/deals/${dealId}`); revalidatePath('/admin/pipeline'); revalidatePath('/deals'); revalidatePath('/dashboard'); redirect(`/deals/${dealId}?saved=selected_offer`); }

...
export async function selectOffer(formData: FormData) { const supabase = await createClient(); const p = Object.fromEntries(formData); const offerId = String(p.offer_id); const dealId = String(p.deal_id); const { data: offer, error: offerError } = await supabase.from('offers').select('id,deal_id').eq('id', offerId).eq('deal_id', dealId).maybeSingle(); if (offerError) throw new Error(offerError.message); if (!offer) throw new Error('Offer not found for this deal'); const { error: resetError } = await supabase.from('offers').update({ status: 'open', selected_at: null }).eq('deal_id', dealId).eq('status', 'accepted'); if (resetError) throw new Error(resetError.message); const { error: offerUpdateError } = await supabase.from('offers').update({ status: 'accepted', selected_at: new Date().toISOString() }).eq('id', offerId); if (offerUpdateError) throw new Error(offerUpdateError.message); const { error: dealError } = await supabase.from('deals').update({ current_stage: toDbPipelineStage('Contracts Out') }).eq('id', dealId); if (dealError) throw new Error(dealError.message); revalidatePath(`/deals/${dealId}`); revalidatePath('/admin/pipeline'); revalidatePath('/deals'); revalidatePath('/dashboard'); redirect(`/deals/${dealId}?saved=selected_offer`); }
